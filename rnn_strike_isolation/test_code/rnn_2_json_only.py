...
class ShotCounter:
    MIN_FRAMES_BETWEEN_SHOTS = 60 #Change this as FPS changes! (defualt is 30)
...
def scan_through_folder(src, dest, model, fps):
    ...
    for file in os.listdir(src):
        if file.endswith("_keypoints.csv"):
            ...
            # Save the JSON file
            json_file_name = os.path.join(dest, file.replace("_keypoints.csv", "_timestamp.json"))
            with open(json_file_name, 'w') as json_file:
                json.dump([json_data], json_file, indent=2)
            print(f"Saving JSON to {json_file_name}")

if __name__ == "__main__":
    source_folder = "C:/Users/Lenovo/Desktop/RNN/new/2"  # This should point to where your CSV files are
    ...
    scan_through_folder(source_folder, output_folder, m1, fps)